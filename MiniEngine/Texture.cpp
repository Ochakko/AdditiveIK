...
void Texture::ReleaseTexture()
{
	...
	//if (!GetExternalReleaseFlag() && m_texture) {//2024/02/07 RenderTargetの場合InitFromD3DResourceで元のテクスチャをreleaseしてm_textureをAddrefしたので、ここでm_textureをreleaseしても良い
	if (m_texture) {
		m_texture->Release();
		m_texture = nullptr;
	}
	...
	//if (m_texture) {
	//	UINT dbgcount = m_texture->Release();
	//	while (dbgcount > 0) {//############ for debug ###########
	//		dbgcount = m_texture->Release();
	//	}
	//	m_texture = nullptr;
	//}
...
int Texture::InitToonFromCustomColor(tag_hsvtoon* phsvtoon)
{
	...
	texbuff->SetName(L"Texture:InitToonFromCustomColor:texbuff");
...
int Texture::InitFromWICFile(const wchar_t* filePath)
{
	...
	texbuff->SetName(L"Texture:InitFromWICFile:texbuff");
