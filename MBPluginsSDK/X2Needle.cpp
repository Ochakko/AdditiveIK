					...
					if ((framecnt >= startframe) && (framecnt <= endframe)) {
						...
						if ((framecnt == startframe) && (framecnt != applyframe)) {
							...
							if (repeatscnt == 0) {
								...
								if (div2) {
									curscale = (float)pow(((double)framecnt - (double)startframe) / (double)halfcnt1, 2.0);//-1.0から1.0
							...
							if (repeatscnt == (repeats - 1)) {
								if (div2) {
									if (minusv) {
										curscale = (float)pow(((double)endframe - (double)framecnt) / (double)halfcnt2, 2.0);
										curscale = (-curscale + 1.0f) * 0.5f;//0.5から1.0　または　0.5から0.0
									}
									else {
										curscale = (float)pow(((double)endframe - (double)framecnt) / (double)halfcnt2, 2.0);//-1.0から1.0
										curscale = (curscale + 1.0f) * 0.5f;//0.0から1.0
									}
								}
								else {
