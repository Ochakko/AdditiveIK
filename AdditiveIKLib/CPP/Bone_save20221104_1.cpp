...
float CBone::CalcAxisMatX_RigidBody(int bindflag, CBone* childbone, ChaMatrix* dstmat, int setstartflag)
{
	...
	//CQuaternion rotq0;
	//ChaMatrix tmprotmat;
	//ChaVector3 upzvec = ChaVector3(0.0f, 0.0f, 0.0f);

	//float dotbefaft;
	//dotbefaft = ChaVector3Dot(&vecx0, &bonevec);
	//if (dotbefaft <= -0.9999f) {
	//	//X軸と重なる剛体が表示されない件に対応
	//	//RotaionArcは180degreeに近い回転に弱く　反対を向いたりする
	//	//１８０degreeに近い場合の処理
	//	rotq0.SetRotationXYZ(0, ChaVector3(0.0f, -180.0f, 0.0f));
	//	tmprotmat = rotq0.MakeRotMatX();
	//	upzvec = vecz0;
	//}
	//else {
	//	rotq0.RotationArc(vecx0, bonevec);
	//	tmprotmat = rotq0.MakeRotMatX();
	//	upzvec = veczm0;
	//}


	//ChaVector3 aftx, afty, aftz;
	//ChaVector3TransformCoord(&aftx, &vecx0, &tmprotmat);
	//ChaVector3TransformCoord(&afty, &vecy0, &tmprotmat);
	//ChaVector3TransformCoord(&aftz, &vecz0, &tmprotmat);

	//ChaVector3Normalize(&aftx, &aftx);
	//ChaVector3Normalize(&afty, &afty);
	//ChaVector3Normalize(&aftz, &aftz);

	////for upvec
	//ChaVector3 aftx1, afty1, aftz1;
	//aftx1 = aftx;
	//ChaVector3Cross(&aftz1, &aftx1, &vecy0);
	//ChaVector3Cross(&afty1, &aftz1, &aftx1);
	//ChaVector3Cross(&aftz1, &aftx1, &afty1);

	//dstmat->SetIdentity();
	//dstmat->data[MATI_11] = aftx1.x;
	//dstmat->data[MATI_12] = aftx1.y;
	//dstmat->data[MATI_13] = aftx1.z;

	//dstmat->data[MATI_21] = afty1.x;
	//dstmat->data[MATI_22] = afty1.y;
	//dstmat->data[MATI_23] = afty1.z;

	//dstmat->data[MATI_31] = aftz1.x;
	//dstmat->data[MATI_32] = aftz1.y;
	//dstmat->data[MATI_33] = aftz1.z;
