...
//		if( curn3p->pervert->createflag != 0 ){
			...
			if( dispv ){
				...
				if (m_fbxfileflag == true) {
					curv->normal = -ChaVector4(curn3p->pervert->smnormal, 0.0f);//2023/07/29 反転
				}
				else {
...
int CPolyMesh3::UpdateMorphBuffer(ChaVector3* mpoint)
{
	...
	//	if (m_fbxfileflag == true) {
	//		dstindex0 = *(m_dispindex + fno * 3 + 2);//!!!!反対周り
	//		dstindex1 = *(m_dispindex + fno * 3 + 1);//!!!!反対周り
	//		dstindex2 = *(m_dispindex + fno * 3);//!!!!反対周り
	//	}
	//	else {
