...
int CBone::UpdateMatrixTargetRate(double srcrate1)
{
	m_targetrate1 = srcrate1;

	return 0;
}

int CBone::UpdateMatrixTarget(bool limitdegflag, int srcmotid, double srcframe, 
	ChaMatrix* wmat, ChaMatrix* vmat, ChaMatrix* pmat, 
	int refposindex)//, int updateslot)
	//default : callingbythread = false, updateslot = 0
{
	...
	if ((g_previewFlag != 5) || (GetParModel() && (GetParModel()->GetBtCnt() == 0))) {
		...
		//modelのworldmatが掛かっていないアニメ姿勢も保存　GetCurrent..., CalcCurrent...用
			m_targetmp.SetAnimMat(newworldmat);


		//modelのworldmatを掛ける
			//skinmeshの変換の際にはシェーダーでg_hmWorldは掛けない　すでにg_hmWorldが掛かっている必要有
			ChaMatrix tmpmat = newworldmat * *wmat; // !!!!!!!!!!!!!!!!!!!!!!!!!!!
			SetWorldMat(limitdegflag, srcmotid, roundingframe, tmpmat, &m_targetmp);//roundingframe!!!!

			if (limitdegflag == true) {
				m_targetmp.SetCalcLimitedWM(2);
			}
			...
			_ASSERT(0);
			m_targetmp.InitParams();
			m_targetmp.SetWorldMat(*wmat);
			m_targetmp.SetFrame(roundingframe);
...
int CBone::BlendSaveBoneMotion(int srcmotid, double srcframe, float srcblend)
{
	...
	if (curmp) {
		...
		ChaMatrix savewm = curmp->GetSaveMotionForFootRig().worldmat;
...
float CBone::CalcAxisMatX_Manipulator(bool limitdegflag, int srcboneaxis, int bindflag, CBone* childbone, ChaMatrix* dstmat, int setstartflag)
{
	...
	ChaVector3 aftparentpos;//2025/05/30 BONEAXIS_PARENTのときのCalcAxisMatX()で使用する
	...
	ChaMatrix tmpzerofm = GetNodeMat() * GetCurrentZeroFrameMat(limitdegflag, 1, multModelWM);
	ChaMatrix tmplimwm = GetNodeMat() * GetCurrentWorldMat(multModelWM, true);
	ChaMatrix tmpbtmat = GetNodeMat() * GetBtMat(true);
	ChaMatrix tmpchildzerofm = childbone->GetNodeMat() * childbone->GetCurrentZeroFrameMat(limitdegflag, 1, multModelWM);
	ChaMatrix tmpchildlimwm = childbone->GetNodeMat() * childbone->GetCurrentWorldMat(multModelWM, true);
	ChaMatrix tmpchildbtmat = childbone->GetNodeMat() * childbone->GetBtMat(true);

	////ChaMatrix tmpchildbtmat;
	//ChaMatrix tmpparentzerofm;
	ChaMatrix tmpparentlimwm;//2025/05/30 BONEAXIS_PARENTのときのCalcAxisMatX()で使用する
	...
	if (GetParent(false)) {//<--- 2024/01/13 parentに対して呼び出すので実質gpar
		if (GetParent(false)->IsSkeleton()) {
			...
			tmpparentlimwm = GetParent(false)->GetNodeMat() * GetParent(false)->GetCurrentWorldMat(multModelWM, true);
			...
			tmpparentlimwm = GetParent(false)->GetTransformMat(GetCurMotID(), curframe, false);
	...
	ChaMatrix convmat;
	convmat.SetIdentity();
	ChaVector3TransformCoord(&aftparentpos, &zeropos, &tmpparentlimwm);//2025/05/30 BONEAXIS_PARENTのときのCalcAxisMatX()で使用する
	...
	if ((srcboneaxis == BONEAXIS_CURRENT) || (srcboneaxis == BONEAXIS_BINDPOSE)) {
		...
		ChaVector3 vecforleng;
		vecforleng = aftbonepos - aftparentpos;
		retleng = (float)ChaVector3LengthDbl(&vecforleng);
	...
	//if (GetParModel() && GetParModel()->GetRokDeBoneUser()) {
		...
		if (srcboneaxis != BONEAXIS_BINDPOSE) {//!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! BINDPOSEの時は　オートフィットしない
			...
			//###########################################################################################
			*dstmat = CalcAxisMatX(bonevec, startpos, convmat);//ChaVecCalc.cpp
		}
		else {
	...
	CBone* childbone, ChaMatrix* dstmat, int setstartflag)
{
	...
	if (bindflag == 1) {
		...
		if ((g_previewFlag != 5) && (g_previewFlag != 4)) {
			...
			ChaMatrix tmplimwm = GetNodeMat() * GetCurrentWorldMat(multModelWM, calcslotflag);
			ChaMatrix tmpchildzerofm = childbone->GetNodeMat() * childbone->GetCurrentZeroFrameMat(limitdegflag, 1, multModelWM);
			//ChaMatrix tmpchildlimwm = childbone->GetNodeMat() * childbone->GetCurrentWorldMat(multModelWM, true);
			ChaMatrix tmpchildlimwm = childbone->GetNodeMat() * childbone->GetCurrentWorldMat(multModelWM, calcslotflag);
...
int CBone::SaveMotionForFootRig(int srcmotid, double srcframe)
{
	double roundingframe = RoundingTime(srcframe);

	//bool calcslotflag = true;
	//m_savebtmat = GetBtMat(calcslotflag);

	CMotionPoint* curmp = GetMotionPoint(srcmotid, roundingframe);
	if (curmp) {
		curmp->SaveMotionForFootRig();
	}

	return 0;
}
int CBone::RestoreMotionForFootRig(int srcmotid, double srcframe)
{

	double roundingframe = RoundingTime(srcframe);

	//bool calcslotflag = true;
	//SetBtMat(m_savebtmat, calcslotflag);

	CMotionPoint* curmp = GetMotionPoint(srcmotid, roundingframe);
	if (curmp) {
		curmp->RestoreMotionForFootRig();
	}
...
ChaMatrix CBone::GetCurrentWorldMat(bool multmodelwm, bool calcslotflag)
{
	...
	//		ChaMatrix retmat = curmpwm * ChaMatrixInv(modelwm);
	//		return retmat;
	//	}
	//	else {
	//		_ASSERT(0);
	...
	if (GetParModel()) {
		if (GetParModel()->ExistCurrentMotion()) {
			...
			if (calcslotflag == true) {//2024/03/15
				...
				curframe = GetParModel()->GetRenderSlotFrame();
			}
...
ChaVector3 CBone::CalcFbxScaleAnim(bool limitdegflag, int srcmotid, double srcframe)
{
	...
	else if(IsNullAndChildIsCamera() || IsCamera()){
		CMotionPoint* curmp = 0;
		curmp = GetMotionPoint(srcmotid, roundingframe);
		if (curmp) {
			ChaMatrix localmat;
...
int CBone::CalcNewBtMat(CModel* srcmodel, CBone* childbone, ChaMatrix* dstmat, ChaVector3* dstpos)
{
	...
	if (IsNotSkeleton()) {
		...
		////2026/07/21
		//*dstmat = GetParModel()->GetWorldMat();
		//*dstpos = ChaMatrixTraVec(*dstmat);
		return 0;
	}
	...
	curworld = GetCurrentWorldMat(true, true);
...
ChaVector3 CBone::GetChildWorld(){
	if (g_previewFlag != 5){
		...
		ChaMatrix tmplimwm = GetCurrentWorldMat(true, true);//2024/03/15
		ChaVector3TransformCoord(&m_childworld, &m_jointfpos, &tmplimwm);
	}
	else{
		ChaMatrix wmat;
		if (GetParent(false) && GetParent(false)->IsSkeleton()){
			wmat = GetParent(false)->GetBtMat(true);
		}
		else{
			wmat = GetBtMat(true);
		}
...
ChaMatrix CBone::GetCurrentZeroFrameMat(bool limitdegflag, int updateflag, bool multModelWM)
{
	//ZeroFrameの編集前と編集後のポーズのdiffをとる必要がある場合に対応する
	//updateflagが1の場合に最新情報。0の場合に前回の取得情報と同じものを返す。

	ChaCalcFunc chacalcfunc;
	return chacalcfunc.GetCurrentZeroFrameMat(this, limitdegflag, updateflag, multModelWM);

}

ChaMatrix CBone::GetCurrentZeroFrameInvMat(bool limitdegflag, int updateflag, bool multModelWM)
{
	//ZeroFrameの編集前と編集後のポーズのdiffをとる必要がある場合に対応する
	//updateflagが1の場合に最新情報。0の場合に前回の取得情報と同じものを返す。

	ChaCalcFunc chacalcfunc;
	chacalcfunc.GetCurrentZeroFrameMat(this, limitdegflag, updateflag, multModelWM);
...
int CBone::CreateIndexedMotionPoint(int srcmotid, double animleng)
{
	...
	//2023/04/28
	if (IsNotSkeleton() && IsNotCamera() && //2023/05/23  2023/06/29
		!IsNullAndChildIsCamera()) {//2024/05/24
		return 0;
	}
...
ChaMatrix CBone::GetTransformMat(int srcmotid, double srctime, bool forceanimflag)
{
	ChaMatrix retmat;
	retmat.SetIdentity();
...
ChaMatrix CBone::CalcFbxLocalMatrix(bool limitdegflag, int srcmotid, double srcframe)
{
	...
	if (parentbone) {
		...
		//
		//	parentfbxwm = parentbone->GetTransformMat(srcmotid, 0.0, true);//2023/07/05 Cameraの子供のスキンメッシュの形が　読み書き読み書き読みテストで形崩れしないように
		//	localfbxmat = GetNodeMat() * wmanim * ChaMatrixInv(parentfbxwm);
		//}
		else {
			parentfbxwm.SetIdentity();
...
ChaMatrix CBone::GetFbxCameraWorldMat(int cameramotid, double nextframe)
{
	//############################################################################
	...
	//camerabone->CalcNodePostureReq(bindposeflag, cameranode, cameramotid, nextframe, &nodemat, &nodeanimmat);
	...
	if ((cameramotid <= 0) || (nextframe < 0.0)) {
		_ASSERT(0);
		transformmat.SetIdentity();
		return transformmat;
	}
	if (!GetParModel()->IsCameraLoaded()) {
		_ASSERT(0);
		transformmat.SetIdentity();
		return transformmat;
	}
	...
	//CAMERANODE* curcamera = GetParModel()->GetCAMERANODE(cameramotid);
	//if (!curcamera) {
	//	_ASSERT(0);
	//	transformmat.SetIdentity();
	//	return transformmat;
	...
	if ((result0 == 0) && camerabone && cameranode) {
		...
		//CalcLocalNodePosture(bindposeflag, 0, cameramotid, nextframe, &parentLocalNodeMat, &tmpParentLocalNodeAnimMat);
		parentGlobalNodeMat = GetTransformMat(cameramotid, roundingframe, true);//global
	}
	else {
		_ASSERT(0);
		parentGlobalNodeMat.SetIdentity();
	...
	//ChaVector3 nodepos = ChaMatrixTraVec(GetNodeMat());
	...
	//switch (g_cameraInheritMode) {//読込時はCAMERA_INHERIT_ALL固定
