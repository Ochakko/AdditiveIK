...
void CPolyMesh4::DestroySystemDispObj(bool emptyshape)
{
	...
	if (m_dispv && emptyshape) {//blend shapeが空では無い場合は消さない
		free(m_dispv);
		m_dispv = 0;
	}
	//if (m_dispindex) {//<----Pick当たり判定で使用するので削除しない
	//	free(m_dispindex);
	//	m_dispindex = 0;
	//}

	if (m_orgindex && emptyshape) {//blend shapeが空では無い場合は消さない
		free(m_orgindex);
		m_orgindex = 0;
	}
	if (m_fbxindex) {
...
int CPolyMesh4::UpdateMorphBuffer( ChaVector3* mpoint )
{
	...
	for (fno = 0; fno < GetFaceNum(); fno++) {
		...
		int dstindex1 = *(m_orgindex + fno * 3 + 2);//!!!!反対周り
		int dstindex2 = *(m_orgindex + fno * 3 + 1);//!!!!反対周り
