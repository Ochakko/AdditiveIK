	...
	if (m_pm4) {
		...
		for (materialcnt = 0; materialcnt < materialnum; materialcnt++) {
			...
			if ((result0 == 0) && (curmat != NULL) && (curtrinum > 0)) {
				...
				m_descriptorHeap.RegistShaderResource(srvNo, curmat->GetAlbedoMap());			//アルベドマップ。
				m_descriptorHeap.RegistShaderResource(srvNo + 1, curmat->GetNormalMap());		//法線マップ。
				m_descriptorHeap.RegistShaderResource(srvNo + 2, curmat->GetSpecularMap());		//スペキュラマップ。
