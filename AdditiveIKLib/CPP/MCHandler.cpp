...
CMCHandler::CMCHandler(CModel* srcparentmodel, int srcfillupleng)
{
	InitParams();
	//m_mh = srcmh;
	m_parentmodel = srcparentmodel;
...
//int CMCHandler::InitMCArray(int srcidlingid)
//{
//
//	if (!m_parentmodel) {
//		_ASSERT(0);
//		return 1;
//	}
//	int motnum = m_parentmodel->GetMotInfoSize();
//
//
//
////DbgOut(  L"check !!! : mchandler : InitMCArray \r\n" );
//	int ret;
//
//	if( !m_mcarray ){
//		m_mcnum = motnum;
...
//		if( !m_mcarray ){
//			DbgOut( L"mchandler : InitMCArray : mcarray alloc error !!!\n" );
//			_ASSERT( 0 );
//			return 1;
//		}
...
//			if( !m_mcarray ){
//				DbgOut(  L"mchandler : InitMCArray : mcarray realloc error !!!\n" );
//				_ASSERT( 0 );
//				return 1;
//			}
//			
...
//
//		}else{
//			DbgOut(  L"mchandler : InitMCArray : motnum error !!!\n" );
//			_ASSERT( 0 );
//			return 1;
//		}
//	}
	...
	int srceventno1, int srcnotfu, int srcnottoidle )
{
	...
	if( !parmce ){
		DbgOut( L"mchandler : AddChild : parmc NULL error !!!\n" );
		_ASSERT( 0 );
		return 1;
	}
	...
	if( !newchildmc ){
		DbgOut( L"mchandler : AddChild : childmc alloc error !!!\n" );
		_ASSERT( 0 );
		return 1;
	}
	...
	int srceventno1, int srcnotfu, int idling, int srcnottoidle )
{
	...
	if( !setmce ){
		DbgOut( L"mchandler : SetMCElem : srcsetno error !!!\n" );
		_ASSERT( 0 );
		return 1;
	}
	...
	if( ret ){
		DbgOut( L"mchandler : SetMCElem : QSortChild error !!!\n" );
		_ASSERT( 0 );
		return 1;
	}
...
int CMCHandler::DeleteChild( MCELEM* parmce, MCELEM* chilmce )
{
	...
	if( !newchild ){
		DbgOut( L"mchandler : DeleteChild : newchild alloc error !!!\n" );
		_ASSERT( 0 );
		return 1;
	}
	...
	if( setno2 != (curchildnum - 1) ){
		DbgOut( L"mchandler : DeleteChild : setno2 error !!!\n" );
		_ASSERT( 0 );
		return 1;
	}
...
int CMCHandler::DeleteMCElem( int srcsetno, bool reordersetnoflag)
{
	...
	if( !delmce ){
		DbgOut( L"mchandler : DeleteMCElem : srcsetno error !!!\n" );
		_ASSERT( 0 );
		return 1;
	}

//DbgOut(  L"check !!! : mchandler : DeleteMCElem : srcsetno %d, delmce->setno %d, delmce->id %d\r\n",
//		srcsetno, delmce->setno, delmce->id );
	...
	if( parmce ){
		...
		if( ret ){
			DbgOut( L"mchandler : Setno2MCElem : DeleteChild error !!!\n" );
			_ASSERT( 0 );
			return 1;
		}
		...
		if( !newmcarray ){
			DbgOut( L"mchandler : DeleteMCElem : newmcarray alloc error !!!\n" );
			_ASSERT( 0 );
			return 1;
		}
		...
		if( setno3 != (m_mcnum - 1 ) ){
			DbgOut( L"mchandler : DeleteMCElem : setno3 error !!!\n" );
			_ASSERT( 0 );
			return 1;
		}

		if( delmce->childmc ){
//DbgOut(  L"check !!! : DeleteMCElem : free delmce->childmc setno %d, id %d\r\n", delmce->setno, delmce->id );
			free( delmce->childmc );
		}
...
//DbgOut(  L"check !!! : mchandler : DeleteMCElem : DeleteChild %d\r\n", delchild->id );

					ret = DeleteChild( curmce, delchild );
					if( ret ){
						DbgOut( L"mchandler : DeleteMCElem : del child : DeleteChild error !!!\n" );
						_ASSERT( 0 );
						return 1;
					}
				}
...
int CMCHandler::AddParentMC( int addcookie, int srcidling, int srcev0idle, int srccommonid, int srcforbidnum, int* srcforbidid, int srcnotfu )
{
	...
	//	DbgOut(  L"mchandler : AddParentMC : addcookie error !!!\n" );
	//	_ASSERT( 0 );
	//	return 1;
	//}
	int ret;
	...
	if( !newarray ){
		DbgOut( L"mchandler : AddParentMC : newarray alloc error !!!\n" );
		_ASSERT( 0 );
		return 1;
	}
	...
	if( ret ){
		DbgOut( L"mch : AddParentMC : SetMCEForbidID error !!!\n" );
		_ASSERT( 0 );
		return 1;
	}
...
DbgOut( L"check !!! : mchandler : AddParentMC : addcookie %d, srcidling %d\r\n",
	addcookie, srcidling );
...
int CMCHandler::SetIdlingMotion(CModel* srcmodel, int exceptfirst)
{
	int ret;

	int idlingid = -1;
	idlingid = GetIdlingMotID(nullptr, 0);
	...
	if( idlingid < 0 ){
		if( exceptfirst ){
			if( m_mcnum >= 2 ){
				...
				DbgOut( L"mchandler : SetIdlingMotion : idling motion not exist error !!!\n" );
				//_ASSERT( 0 );
				return 1;//!!!!!!!!!
			}
		}else{
	...
	ret = srcmodel->SetCurrentMotion(idlingid);
	if( ret ){
		DbgOut( L"mchandler : SetIdlingMotion : mh SetMotionKind error !!!\n" );
		_ASSERT( 0 );
		return 1;
	}
	...
	//		DbgOut(  L"mchandler : SetIdlingMotionNext : mh SetNextMotionFrameNo error !!!\n" );
	//		_ASSERT( 0 );
	//		return 1;
	//	}
	//}
...
int CMCHandler::GetIdlingMotID(char* dstname, int dstleng)
{
	if (dstname) {
		*dstname = 0;
	}
	if ((dstleng < 0) && (dstleng > 4098)) {
		_ASSERT(0);
		return -1;
	}
	if (!m_parentmodel) {
	...
	for( mcno = 0; mcno < m_mcnum; mcno++ ){
		...
		if( curmce->idling == 1 ){
			idlingid = curmce->id;
			MOTINFO idlingmi = m_parentmodel->GetMotInfo(idlingid);
			if (idlingmi.motid > 0) {
				int nameleng = (int)strlen(idlingmi.motname);
				if (dstname && (nameleng > 0) && (nameleng < dstleng)) {
...
int CMCHandler::ChangeIdlingMotion(CModel* srcmodel, int srcmotid)
{
	int mcno;
	for( mcno = 0; mcno < m_mcnum; mcno++ ){
		MCELEM* curmce;
	...
	int* nextmotidptr, int* nextframeptr, int* notfuptr, int* nottoidleptr )
{
	...
	if( !parentmce ){
		DbgOut(  L"mch : GetNextMotion : parentmce NULL error !!!\n" );
		_ASSERT( 0 );
		return 1;
	}
...
//if( srceventno != 0 ){
//	DbgOut(  L"mchandler : GetNextMotion : userevent %d, nextmotid %d, nextframe %d\r\n",
...
int CMCHandler::GetTrunkNotComID( int motid, int arrayleng, int* dstid, int* getnum )
{
	...
	if( !parentmce ){
		DbgOut(  L"mch : GetTrunkNotComID : parentmce NULL return !!!\n" );
		//_ASSERT( 0 );
		*getnum = 0;
		return 1;
	}
	...
	*getnum = parentmce->forbidnum;
	...
	if( (arrayleng > 0) && (arrayleng < parentmce->forbidnum) ){
		DbgOut(  L"mch : GetTrunkNotComID : arrayleng too short error !!!\n" );
		_ASSERT( 0 );
		return 1;
	}
...
int CMCHandler::SetTrunkNotComID( int motid, int arrayleng, int* srcid )
{
	...
	if( !parentmce ){
		DbgOut(  L"mch : SetTrunkNotComID : parentmce NULL return !!!\n" );
		//_ASSERT( 0 );
		return 1;
	}
	...
	if( !parentmce->forbidid ){
		DbgOut(  L"mch : SetTrunkNotComID : forbidid alloc error !!!\n" );
		_ASSERT( 0 );
		return 1;
	}
...
int CMCHandler::GetTrunkInfo( int motid, MOATRUNKINFO* trunkptr )
{
	...
	if( !parentmce ){
		DbgOut(  L"mch : GetTrunkInfo : parentmce NULL return !!!\n" );
		//_ASSERT( 0 );
		trunkptr->idling = 0;
		trunkptr->ev0idle = 0;
		trunkptr->comid = 0;
...
int CMCHandler::GetBranchInfo( int motid, MOABRANCHINFO* branchptr, int branchnum, int* getnum )
{
	...
	if( !parentmce ){
		DbgOut(  L"mch : GetBranchInfo : parentmce NULL return !!!\n" );
		//_ASSERT( 0 );
		*getnum = 0;
		return 0;
	}
	...
	if( branchnum < parentmce->childnum ){
		DbgOut(  L"mch : GetBranchInfo : branchnum too short error !!!\n" );
		_ASSERT( 0 );
		return 1;
	}
...
int CMCHandler::SetBranchFrame1( int tmotid, int bmotid, int frame1 )
{
	...
	if( !parentmce ){
		DbgOut(  L"mch : SetBranchFrame1 : parentmce NULL return !!!\n" );
		_ASSERT( 0 );
		return 0;//!!!!!!!!!!!!!!!!!!!!!
	}
	...
	if( !chilmce ){
		DbgOut(  L"mch : SetBranchFrame1 : chilmce NULL return !!!\n" );
		_ASSERT( 0 );
		return 0;
	}
...
int CMCHandler::SetBranchFrame2( int tmotid, int bmotid, int frame2 )
{
	...
	if( !parentmce ){
		DbgOut(  L"mch : SetBranchFrame2 : parentmce NULL return !!!\n" );
		_ASSERT( 0 );
		return 0;//!!!!!!!!!!!!!!!!!!!!!
	}
	...
	if( !chilmce ){
		DbgOut(  L"mch : SetBranchFrame2 : chilmce NULL return !!!\n" );
		_ASSERT( 0 );
		return 0;
	}
