		...
		public:			
			...
			void draw(HDCMaster *hdcM,
				...
				for (int i = 0; i < (int)key.size(); i++) {
					...
					int ey0 = (parent->maxeul - key[i]->value * parent->getDispScale()) / eulrange * (y1 - y0) + y0;
				...
				if (wcscmp(L"X", name.c_str()) == 0) {
					...
					double mesurestep;
					if (parent->ikkind == 0) {
						mesurestep = 10.0 / parent->getDispScale();
					}
					else if (parent->ikkind == 1) {
						mesurestep = 2.0 / parent->getDispScale();
					}
					else {
						mesurestep = 10.0 / parent->getDispScale();
					}
					...
					int ey0 = (int)((parent->maxeul - minmeasure * parent->getDispScale()) / eulrange * (y1 - y0) + y0);
					int ex0 = (int)(x0 + parent->LABEL_SIZE_X - 7 * fontsize);

					WCHAR strmeasure[64];
					swprintf_s(strmeasure, 64, L"%+.2lf---", minmeasure);
					TextOut(hdcM->hDC,
						ex0, ey0,
					...
					ey0 = (int)((parent->maxeul - maxmeasure * parent->getDispScale()) / eulrange * (y1 - y0) + y0);//
					...
					swprintf_s(strmeasure, 64, L"%+.2lf---", maxmeasure);
					TextOut(hdcM->hDC,
						ex0, ey0,
						strmeasure, (int)wcslen(strmeasure));
					hdcM->setPenAndBrush(RGB(min(baseR + 20, 255), min(baseG + 20, 255), min(baseB + 20, 255)), NULL);
					...
					for (int curmeasure = minmeasure + mesurestep; curmeasure < maxmeasure; curmeasure += mesurestep) {
						...
						ey0 = (int)((parent->maxeul - curmeasure * parent->getDispScale()) / eulrange * (y1 - y0) + y0);
						ex0 = (int)(x0 + parent->LABEL_SIZE_X - 7 * fontsize);

						//if (abs(ey0 - befey0) > 20) {
						if (abs(ey0 - befey0) > (mesurestep * 2)) {
							swprintf_s(strmeasure, 64, L"%+.2lf---", curmeasure);
